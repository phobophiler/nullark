...
describe("EncryptedOutputNoteV2 envelope", () => {
  it("encrypts compact V2 output-note payloads that fit the fixed padded ciphertext length", async () => {
    const recoveryKey = await deriveWalletRecoveryKey({
      walletSignature: signature,
      chainId: 6343,
      pool,
    ...
    const compact = await encryptCompactOutputNoteV2Payload({ noteKey, plaintext, aad });
    const output = createEncryptedOutputNoteV2Envelope({
      chainId: 6343,
      verifyingContract: pool,
      outputCommitment: commitment,
      nonce: compact.nonce,
      ciphertext: compact.ciphertext
    });

    expect(output.ciphertextByteLength).toBeLessThanOrEqual(256);
    expect(output.nonce).toBe(compact.nonce);
    expect(output.paddedCiphertextByteLength).toBe(256);
  });

  it("accepts only the fixed-shape padded withdraw-output action", () => {
    const output = createEncryptedOutputNoteV2Envelope({
      chainId: 6343,
      verifyingContract: pool,
      outputCommitment: commitment,
    ...
    const serialized = serializeEncryptedOutputNoteV2EnvelopeToHex(output);
    ...
    expect(hexToUtf8(serialized)).not.toMatch(/wallet|tag/i);
  });

  it("rejects malformed missing or empty V2 output envelopes", () => {
    expect(() => parseEncryptedOutputNoteV2EnvelopeFromHex("0xabcd")).toThrow(
      "EncryptedOutputNoteV2 envelope is not valid JSON."
    );
    expect(() =>
      parseEncryptedOutputNoteV2EnvelopeFromHex(utf8ToHex(JSON.stringify({ version: 2 })))
    ).toThrow("EncryptedOutputNoteV2 envelope domain must be nullark.encrypted-output-note.v2.");
    expect(() =>
      createEncryptedOutputNoteV2Envelope({
        chainId: 6343,
        verifyingContract: pool,
    ...
    ).toThrow("EncryptedOutputNoteV2 ciphertext must be nonempty even-length hex bytes.");
  });
  ...
  it("rejects stale V1 amount, spent-commitment, and change-labeled fields on V2 output envelopes", () => {
    ...
    const staleOutput = {
      ...output,
      noteAmount: "5000000000000000",
      oldNoteAmount: "5000000000000000",
      outputAmount: "0",
      spentCommitment: `0x${"99".repeat(32)}`,
      changeAmount: "0"
    };

    expect(() => parseEncryptedOutputNoteV2EnvelopeFromHex(utf8ToHex(JSON.stringify(staleOutput)))).toThrow(
      "EncryptedOutputNoteV2 envelope has unexpected field noteAmount."
    );
  });
});
...
describe("EncryptedNoteV1 envelope", () => {
  ...
  it("rejects malformed EncryptedNoteV1 payload fields and ambiguous encodings", () => {
    ...
    expect(() => validateEncryptedNoteV1Envelope({ ...encryptedNoteV1, action: "send" })).toThrow(
      "EncryptedNoteV1 action must be deposit, private-transfer, withdraw, withdraw-change, or withdraw-output."
    );
