...
export type EncryptedOutputNoteV2Ciphertext = {
  ciphertext: HexString;
  nonce?: HexString | undefined;
  ephemeralPublicKey?: HexString | undefined;
};
...
export async function encryptCompactOutputNoteV2Payload(input: {
  noteKey: CryptoKey;
  plaintext: SpendMaterialPlaintext;
  aad: Uint8Array;
}): Promise<EncryptedOutputNoteV2Ciphertext & { nonce: HexString }> {
  const nonce = crypto.getRandomValues(new Uint8Array(24));
  const encodedPlaintext = textEncoder.encode(
    JSON.stringify([
      input.plaintext.noteAmountWei,
      input.plaintext.ownerCommitment.slice(2),
      input.plaintext.noteSecret.slice(2),
      input.plaintext.blinding.slice(2)
    ])
  );
  ...
  const encrypted = await crypto.subtle.encrypt(
    ...
    input.noteKey,
    asArrayBuffer(encodedPlaintext)
  );
...
export function createEncryptedOutputNoteV2Envelope(input: {
  ...
  const ciphertext = requireV2NonEmptyHex(input.ciphertext, "ciphertext");
  ...
  if (ciphertextByteLength > paddedCiphertextByteLength) {
    throw new Error("EncryptedOutputNoteV2 ciphertext exceeds fixed padded length.");
  }
  ...
  return {
    ...
    chainId: requireV2ChainId(input.chainId),
    verifyingContract: requireV2Address(input.verifyingContract, "verifyingContract"),
    action: "withdraw-output",
    outputCommitment: requireV2Bytes32(input.outputCommitment, "outputCommitment"),
    proofContextHash: ZERO_BYTES32,
    ephemeralPublicKey: input.ephemeralPublicKey ? requireV2Bytes32(input.ephemeralPublicKey, "ephemeralPublicKey") : ZERO_BYTES32,
    nonce: input.nonce ? requireV2Bytes24(input.nonce, "nonce") : ZERO_BYTES24,
    ciphertext,
    ciphertextByteLength,
...
export function parseEncryptedOutputNoteV2EnvelopeFromHex(
  ...
  try {
    ...
    throw new Error("EncryptedOutputNoteV2 envelope is not valid JSON.");
  }

  return validateEncryptedOutputNoteV2Envelope(parsed, checks);
}
...
function requireEncryptedNoteV1Action(value: unknown): EncryptedNoteV1Action {
  if (typeof value !== "string" || !ENCRYPTED_NOTE_V1_ACTIONS.has(value as EncryptedNoteV1Action)) {
    throw new Error("EncryptedNoteV1 action must be deposit, private-transfer, withdraw, withdraw-change, or withdraw-output.");
  }
...
function validateEncryptedOutputNoteV2Envelope(
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error("EncryptedOutputNoteV2 envelope JSON must be an object.");
  }

  const envelope = value as Record<string, unknown>;
  for (const key of Object.keys(envelope)) {
    if (!(ENCRYPTED_OUTPUT_NOTE_V2_FIELDS as readonly string[]).includes(key)) {
      throw new Error(`EncryptedOutputNoteV2 envelope has unexpected field ${key}.`);
    }
  }
  const normalized: EncryptedOutputNoteV2Envelope = {
    version: requireV2Version(envelope.version),
    domain: requireV2Domain(envelope.domain),
    chainId: requireV2ChainId(envelope.chainId),
    verifyingContract: requireV2Address(envelope.verifyingContract, "verifyingContract"),
    action: requireV2Action(envelope.action),
    outputCommitment: requireV2Bytes32(envelope.outputCommitment, "outputCommitment"),
    proofContextHash: requireV2Bytes32(envelope.proofContextHash, "proofContextHash"),
    ephemeralPublicKey: requireV2Bytes32(envelope.ephemeralPublicKey, "ephemeralPublicKey"),
    nonce: requireV2Bytes24(envelope.nonce, "nonce"),
    ciphertext: requireV2NonEmptyHex(envelope.ciphertext, "ciphertext"),
    ciphertextByteLength: requireV2SafeInteger(envelope.ciphertextByteLength, "ciphertextByteLength"),
    paddingBytes: requireV2Hex(envelope.paddingBytes, "paddingBytes"),
    paddingByteLength: requireV2SafeInteger(envelope.paddingByteLength, "paddingByteLength"),
    paddedCiphertextByteLength: requireV2SafeInteger(envelope.paddedCiphertextByteLength, "paddedCiphertextByteLength")
  };

  if (hexByteLength(normalized.ciphertext) !== normalized.ciphertextByteLength) {
    throw new Error("EncryptedOutputNoteV2 ciphertextByteLength must match ciphertext bytes.");
  }
  if (hexByteLength(normalized.paddingBytes) !== normalized.paddingByteLength) {
    throw new Error("EncryptedOutputNoteV2 paddingByteLength must match padding bytes.");
  }
  if (normalized.ciphertextByteLength + normalized.paddingByteLength !== normalized.paddedCiphertextByteLength) {
    throw new Error("EncryptedOutputNoteV2 padded length must equal ciphertext plus padding.");
  }
  if (normalized.paddedCiphertextByteLength !== ENCRYPTED_OUTPUT_NOTE_V2_PADDED_CIPHERTEXT_BYTES) {
    throw new Error("EncryptedOutputNoteV2 paddedCiphertextByteLength must be 256.");
  }
  if (!/^0x(?:00)*$/.test(normalized.paddingBytes)) {
    throw new Error("EncryptedOutputNoteV2 paddingBytes must be zero padding.");
  }
  if (checks.chainId !== undefined && normalized.chainId !== checks.chainId) {
    throw new Error("EncryptedOutputNoteV2 chainId does not match the active chain.");
  }
  if (
    checks.verifyingContract !== undefined &&
    normalized.verifyingContract !== checks.verifyingContract.toLowerCase()
  ) {
    throw new Error("EncryptedOutputNoteV2 verifyingContract does not match the configured pool.");
  }
  if (checks.outputCommitment !== undefined && normalized.outputCommitment !== checks.outputCommitment.toLowerCase()) {
    throw new Error("EncryptedOutputNoteV2 outputCommitment does not match the withdrawal output.");
  }

  return normalized;
}

function requireV2Version(value: unknown): 2 {
  if (value !== 2) {
    throw new Error("Unsupported EncryptedOutputNoteV2 envelope version.");
  }
  return 2;
}

function requireV2Domain(value: unknown): typeof ENCRYPTED_OUTPUT_NOTE_V2_DOMAIN {
  if (value !== ENCRYPTED_OUTPUT_NOTE_V2_DOMAIN) {
    throw new Error("EncryptedOutputNoteV2 envelope domain must be nullark.encrypted-output-note.v2.");
  }
  return ENCRYPTED_OUTPUT_NOTE_V2_DOMAIN;
}

function requireV2ChainId(value: unknown): 4326 | 6343 {
  if (value !== 4326 && value !== 6343) {
    throw new Error("EncryptedOutputNoteV2 chainId must be MegaETH testnet 6343 or mainnet 4326.");
  }
  return value;
}

function requireV2Action(value: unknown): "withdraw-output" {
  if (value !== "withdraw-output") {
    throw new Error("EncryptedOutputNoteV2 envelope action must be withdraw-output.");
  }
  return "withdraw-output";
}

function requireV2Address(value: unknown, fieldName: string): HexString {
  if (typeof value !== "string" || !ADDRESS_PATTERN.test(value)) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be an EVM address.`);
  }
  return value.toLowerCase() as HexString;
}

function requireV2Bytes32(value: unknown, fieldName: string): HexString {
  if (typeof value !== "string" || !BYTES32_PATTERN.test(value)) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be bytes32 hex.`);
  }
  return value.toLowerCase() as HexString;
}

function requireV2Bytes24(value: unknown, fieldName: string): HexString {
  if (typeof value !== "string" || !/^0x[0-9a-fA-F]{48}$/.test(value)) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be bytes24 hex.`);
  }
  return value.toLowerCase() as HexString;
}

function requireV2Hex(value: unknown, fieldName: string): HexString {
  if (typeof value !== "string" || !/^0x(?:[0-9a-fA-F]{2})*$/.test(value)) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be even-length hex bytes.`);
  }
  return value.toLowerCase() as HexString;
}

function requireV2NonEmptyHex(value: unknown, fieldName: string): HexString {
  if (typeof value !== "string" || !EVEN_HEX_PATTERN.test(value)) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be nonempty even-length hex bytes.`);
  }
  return value.toLowerCase() as HexString;
}

function requireV2SafeInteger(value: unknown, fieldName: string): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) {
    throw new Error(`EncryptedOutputNoteV2 ${fieldName} must be a nonnegative safe integer.`);
  }
