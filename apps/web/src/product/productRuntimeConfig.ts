...
const MAINNET_PUBLIC_APPROVAL_SOURCE_HASH =
...
const MAINNET_VALUE_MOVING_APPROVAL_ARTIFACT_HASHES: readonly HexString[] = [
  MAINNET_PUBLIC_APPROVAL_SOURCE_HASH
];
...
export const MAINNET_GUARDED_USERS_BLOCKED_MESSAGE =
  "MegaETH mainnet user actions are blocked until guarded-user approval is explicitly enabled.";
...
function getProductRuntimeConfigFromEnv(): ProductRuntimeConfig | null {
  ...
  const relayerEndpoint = mainnetValueMovingApproved && guardedUsersApproved
...
export function isMainnetGuardedUsersBlocked(config: ProductRuntimeConfig): boolean {
  return config.chainId === MAINNET_CHAIN_ID && !config.guardedUsersApproved;
}

export function isMainnetUserActionBlocked(config: ProductRuntimeConfig): boolean {
  return isMainnetValueMovingBlocked(config) || isMainnetGuardedUsersBlocked(config);
}
...
export function assertMainnetValueMovingAllowed(config: ProductRuntimeConfig): void {
  ...
  if (isMainnetGuardedUsersBlocked(config)) {
    throw new Error(MAINNET_GUARDED_USERS_BLOCKED_MESSAGE);
  }
}
