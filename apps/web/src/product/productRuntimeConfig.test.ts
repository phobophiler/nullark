...
const MAINNET_VALUE_MOVING_APPROVAL_HASH =
  "0xb87aa47a407f0347a920fcebe76f84d402be8bd5e82f5fe5980ffea557bfa996";
...
describe("product runtime config", () => {
  ...
  it("uses the promoted v1.2 mainnet pool config by default", () => {
    ...
    expect(isMainnetUserActionBlocked(getProductRuntimeConfig())).toBe(true);
  });
  ...
  it("keeps mainnet user actions blocked when only value-moving approval is pinned", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
      relayerEndpoint: MAINNET_RELAYER,
      mainnetValueMovingApproved: true,
    ...
    expect(config.relayerEndpoint).toBeUndefined();
    expect(isMainnetValueMovingBlocked(config)).toBe(false);
    expect(isMainnetUserActionBlocked(config)).toBe(true);
    expect(() => assertMainnetValueMovingAllowed(config)).toThrow(MAINNET_GUARDED_USERS_BLOCKED_MESSAGE);
    expect(() => assertProductionPrivacyClaimsAllowed(config)).toThrow(PRODUCTION_PRIVACY_CLAIMS_BLOCKED_MESSAGE);
  });

  it("enables mainnet user actions when value-moving and guarded-user approvals are hash pinned", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
      relayerEndpoint: MAINNET_RELAYER,
      mainnetValueMovingApproved: true,
    ...
    expect(isMainnetValueMovingBlocked(config)).toBe(false);
    expect(isMainnetUserActionBlocked(config)).toBe(false);
  ...
  it("accepts the trusted setup record hash as guarded-user approval evidence", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
      mainnetValueMovingApproved: true,
      mainnetValueMovingApprovalHash: MAINNET_VALUE_MOVING_APPROVAL_HASH
  ...
  it("does not enable legacy relayer endpoints without a distinct guarded-user approval artifact", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
    ...
    expect(() => getProductRuntimeConfig()).toThrow("VITE_RELAYER_ENDPOINT to target /transaction");
  });

  it("does not enable placeholder relayer endpoints without a distinct guarded-user approval artifact", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
    ...
    expect(() => getProductRuntimeConfig()).toThrow("placeholder relayer endpoint");
  });

  it("does not enable noncanonical relayer hosts without a distinct guarded-user approval artifact", () => {
    setLocation("https://shielded-private-transfer.pages.dev/");
    stubMainnetEnv({
