...
export function ShieldedTransfersPanel() {
  ...
  const valueMovingDisabled = actionInFlight || mainnetValueMovingBlocked || publicRuntimeValueMovingBlocked;
    ...
    try {
      ...
      setNoteRecordStatus(
        record.recoveryRoute === "recovery-kit"
          ? "Recovery kit imported. Enter a recipient and withdraw; the relayer will submit it when ready."
  ...
  const attentionToastMessage = error || (mainnetValueMovingBlocked ? MAINNET_VALUE_MOVING_BLOCKED_MESSAGE : "");
  const showAttentionToast = Boolean(attentionToastMessage) && dismissedAttentionToastMessage !== attentionToastMessage;
  const attentionToastTitle = mainnetValueMovingBlocked
    ? "Mainnet blocked"
