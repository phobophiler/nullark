...
export async function recoverWalletNotesFromChain(input: {
  ...
  const recoveryContext = { recoveryKeys, chainId, rpcUrl, pool, currentRoot, fetchFn };
  const recovered: RecoveredWalletNoteEntry[] = [];
  for (const log of logs) {
    const recoveredEntry = await recoverWalletNoteFromLogWithContext(log, recoveryContext);
    if (recoveredEntry) {
...
export async function recoverWalletNoteFromLog(input: {
  walletSignature: HexString;
  log: RawRpcLog;
  chainId?: SupportedMegaEthChainId;
  rpcUrl?: string;
  pool?: HexString;
  currentRoot?: HexString | null;
  fetchFn?: typeof fetch;
}): Promise<RecoveredWalletNoteEntry | null> {
  const fetchFn = input.fetchFn ?? fetch;
  if (input.chainId === undefined || input.rpcUrl === undefined || input.pool === undefined) {
    throw new Error("Live note recovery requires chainId, rpcUrl, and pool.");
  }
  if (!isSupportedMegaEthChainId(input.chainId)) {
    throw new Error("Expected live note recovery chain ID to be MegaETH mainnet or testnet.");
  }
  if (!isEvmAddress(input.pool)) {
    throw new Error("Expected live note recovery pool to be an EVM address.");
  }
  if (!/^https:\/\/[^ ]+$/i.test(input.rpcUrl)) {
    throw new Error("Expected live note recovery RPC URL to be HTTPS.");
  }
  ...
  const currentRoot =
    input.currentRoot ??
    (await rpcRequest<string>(fetchFn, "eth_call", [{ to: pool, data: CURRENT_ROOT_CALLDATA }, "latest"], rpcUrl).catch(
      () => null
    ));
  const recoveryKeys = await Promise.all(
    [NULLARK_RECOVERY_APP_ID, LEGACY_SHIELDED_TRANSFERS_RECOVERY_APP_ID]
  ...
  return recoverWalletNoteFromLogWithContext(input.log, { recoveryKeys, chainId, rpcUrl, pool, currentRoot, fetchFn });
}

async function recoverWalletNoteFromLogWithContext(
  log: RawRpcLog,
  input: {
  ...
  try {
    ...
    const plaintext = await tryDecryptEvent({
      event,
    ...
    const derivedCommitment =
      event.action === "withdraw-output" && event.encryptionVersion === 2
        ? await deriveBrowserOutputNoteCommitment({
        ...
        : await deriveBrowserNoteCommitment({
            assetId: plaintext.assetId,
            noteAmountWei: plaintext.noteAmountWei,
            ownerCommitment: plaintext.ownerCommitment,
            noteSecret: plaintext.noteSecret
    ...
    const spent = await rpcRequest<string>(
      input.fetchFn,
      "eth_call",
      [{ to: input.pool, data: encodeNullifierLookupCalldata(spendNullifier) }, "latest"],
      input.rpcUrl
    )
    ...
    return {
      record: createSandboxSpendMaterialNoteRecord({
        ...
        depositBlockNumber: isBlockQuantity(log.blockNumber) ? log.blockNumber : null,
        currentRootAfter: input.currentRoot && isHexBytes32(input.currentRoot) ? input.currentRoot : null,
...
async function tryDecryptEvent(input: {
  ...
  if (input.event.action === "withdraw-output" && input.event.encryptionVersion === 2) {
    return tryDecryptV2OutputEvent(input);
  }
...
async function tryDecryptV2OutputEvent(input: {
  event: DecodedNoteEvent;
  recoveryKeys: CryptoKey[];
  chainId: SupportedMegaEthChainId;
  pool: HexString;
  ...
  const encrypted = (() => {
    try {
      return parseEncryptedOutputNoteV2EnvelopeFromHex(input.event.encryptedNote, {
        chainId: input.chainId,
        verifyingContract: input.pool,
  ...
  for (const recoveryKey of input.recoveryKeys) {
    for (const epochId of epochIds) {
      try {
        ...
        return await decryptCompactOutputNoteV2Payload({
          noteKey,
...
async function decryptCompactOutputNoteV2Payload(input: {
  noteKey: CryptoKey;
  nonce: HexString;
  ...
  const decrypted = await crypto.subtle.decrypt(
    {
      name: "AES-GCM",
      iv: asArrayBuffer(hexToBytes(input.nonce).slice(0, 12)),
      additionalData: asArrayBuffer(input.aad)
    },
    input.noteKey,
    asArrayBuffer(hexToBytes(input.ciphertext))
  );
  ...
  if (
    ...
    !/^(0|[1-9][0-9]*)$/.test(parsed[0]) ||
    !/^[0-9a-fA-F]{64}$/.test(parsed[1]) ||
    !/^[0-9a-fA-F]{64}$/.test(parsed[2]) ||
    !/^[0-9a-fA-F]{64}$/.test(parsed[3])
  ) {
    throw new Error("EncryptedOutputNoteV2 inner note payload is malformed.");
  }
  ...
  return {
    ...
    noteAmountWei: parsed[0],
    ownerCommitment: `0x${parsed[1].toLowerCase()}`,
    noteSecret: `0x${parsed[2].toLowerCase()}`,
    blinding: `0x${parsed[3].toLowerCase()}`,
  ...
  try {
    ...
    if (decoded.eventName === "DepositNoteCreated") {
      return {
        ...
        encryptionVersion: Number(decoded.args.encryptionVersion)
      };
    }
