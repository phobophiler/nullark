...
import { WALLET_RECOVERY_SCOPE_ISSUED_AT } from "../recovery/walletUnlock.js";
import {
...
function createExplicitMainnetProductRuntimeConfig(): ProductRuntimeConfig {
  return {
    ...
    networkName: "MegaETH",
    networkBadge: "MegaETH",
    walletChainName: "MegaETH",
...
async function restoreRecoveryKitThroughVisiblePanel(record: ReturnType<typeof createSandboxSpendMaterialNoteRecord>) {
  ...
  fireEvent.click(screen.getByRole("checkbox", { name: /bearer secret/i }));
...
describe("ShieldedTransfersPanel", () => {
  it("renders only the Nullark deposit and public exit actions by default", () => {
    ...
    expect(screen.queryByText("MegaETH Testnet")).not.toBeInTheDocument();
  ...
  it("renders the MegaETH network label without a mainnet switch affordance", () => {
    ...
    expect(
      screen.queryByText("Production privacy claims are disabled until a public privacy-claims artifact is explicitly enabled.")
  ...
  it("keeps mainnet controls blocked until guarded users are explicitly approved too", () => {
    setProductRuntimeConfigForTests({
      ...createExplicitMainnetProductRuntimeConfig(),
      mainnetValueMovingApproved: true
    });
    ...
    expect(
      screen.getByText("MegaETH mainnet user actions are blocked until guarded-user approval is explicitly enabled.")
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Deposit" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Withdraw" })).toBeDisabled();
  });

  it("enables mainnet controls only when both mainnet approvals are explicit", () => {
    setProductRuntimeConfigForTests({
      ...createExplicitMainnetProductRuntimeConfig(),
      mainnetValueMovingApproved: true,
    ...
    expect(screen.queryByText(/mainnet .* blocked until/i)).not.toBeInTheDocument();
  ...
  it("keeps the testnet query override accessible without a visible switcher", () => {
    setProductRuntimeConfigForTests(null);
  ...
  it("rechecks and switches the wallet chain immediately before sending a deposit transaction", async () => {
    const account = "0x1111111111111111111111111111111111111111";
    const depositTxHash = `0x${"31".repeat(32)}` as const;
    const expectedChainIdHex = `0x${MEGAETH_TESTNET_CHAIN_ID.toString(16)}`;
    let activeChainId = expectedChainIdHex;
    ...
    await waitFor(() => expect(request).toHaveBeenCalledWith(expect.objectContaining({ method: "eth_sendTransaction" })));
    const methods = request.mock.calls.map(([args]) => args.method);
    const sendIndex = methods.indexOf("eth_sendTransaction");
    expect(methods.slice(sendIndex - 2, sendIndex + 1)).toEqual([
      "eth_chainId",
      "wallet_switchEthereumChain",
      "eth_sendTransaction"
    ]);
    expect(activeChainId).toBe(expectedChainIdHex);
  });
  ...
  it("deduplicates Rabby when EIP-6963 and injected providers both announce it", async () => {
    const rabbyRequest = vi.fn(async ({ method }) => {
      if (method === "eth_requestAccounts") {
    ...
    const rabbyEip6963Provider = { isRabby: true, request: rabbyRequest };
    const rabbyInjectedProvider = { isRabby: true, request: rabbyRequest };
    Object.defineProperty(window, "ethereum", {
      configurable: true,
      value: {
        providers: [
          { isMetaMask: true, request: vi.fn() },
          rabbyInjectedProvider
        ]
      }
    });
    const dispatchEvent = window.dispatchEvent.bind(window);
    ...
    await waitFor(() => expect(screen.getByRole("option", { name: "Rabby Wallet" })).toBeInTheDocument());
    expect(screen.queryByRole("option", { name: "Rabby" })).not.toBeInTheDocument();
  });
  ...
  it("requires bearer-secret acknowledgement and clears recovery kit text after failed import", async () => {
    render(<ShieldedTransfersPanel />);
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(/Recovery kit|JSON|Invalid/i);
    expect(screen.getByLabelText("Recovery kit JSON")).toHaveValue("");
  });
  ...
  it("always submits guided public exits through the local relayer instead of the user wallet", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(
      expect.stringContaining("Final withdrawal relay review")
    );
    const reviewText = String(confirmMock.mock.calls.at(-1)?.[0] ?? "");
    expect(reviewText).toContain(`Chain ID: ${MEGAETH_TESTNET_CHAIN_ID}`);
    expect(reviewText).toContain(`Recipient: ${destination}`);
    expect(reviewText).toContain("Gross amount: 0.00001 ETH");
    expect(reviewText).toContain("Net amount: 0.000009967 ETH");
    expect(reviewText).toContain("Fee: 0.000000033 ETH");
    expect(reviewText).toContain("Max fee: 0.000000033 ETH");
    expect(reviewText).toContain(`Pool: ${SHIELDED_POOL_ADDRESS}`);
    expect(reviewText).toContain("Relayer endpoint: https://testnet-relayer.nullark.com/transaction");
    expect(reviewText).toContain("Output/change note handling:");
  ...
  it("uses a verified recovery hint block to bound browser RootAccepted reconstruction", async () => {
    const hintedBlock = "0x77" as const;
    const noteRecord = createSandboxSpendMaterialNoteRecord({
      ...
      blinding: DETERMINISTIC_SPEND_MATERIAL_FIELD,
      depositTxHash: `0x${"ab".repeat(32)}`,
      depositBlockNumber: hintedBlock,
      currentRootAfter: `0x${"12".repeat(32)}`,
    ...
    setDiagnosticsLocation();
    const destination = "0x1111111111111111111111111111111111111111";
    const withdrawTxHash = `0x${"0a".repeat(32)}`;
        ...
        if (key === MEGAETH_TESTNET_RPC_URL) {
          ...
          if (body.method === "eth_getTransactionReceipt") {
            return Response.json({
              ...
              id: body.id,
              result: {
                blockNumber: hintedBlock,
                logs: [{ address: SHIELDED_POOL_ADDRESS, topics: [DEFAULT_AMOUNT_POSEIDON_COMMITMENT] }]
              }
            });
          }
          if (body.method === "eth_getLogs") {
            logFilters.push(body.params?.[0] ?? {});
            return Response.json({ jsonrpc: "2.0", id: body.id, result: rootAcceptedLogsFor([DEFAULT_AMOUNT_POSEIDON_COMMITMENT]) });
          }
          return Response.json({ jsonrpc: "2.0", id: body.id, result: `0x${"0".repeat(63)}1` });
        }
        if (key === "http://127.0.0.1:63430/transaction") {
          return Response.json({
            ok: true,
            ...
            receipt: {
              ...
              from: "0x1111111111111111111111111111111111111111",
              to: SHIELDED_POOL_ADDRESS,
              logs: [{ address: SHIELDED_POOL_ADDRESS, topics: [withdrawProofWorkerRequests[0]?.witness.nullifier ?? `0x${"00".repeat(32)}`] }]
            }
          });
        }
        return new Response("missing", { status: 404 });
    ...
    await waitFor(() => expect(withdrawProofWorkerRequests.length).toBeGreaterThan(0), { timeout: 4000 });
  ...
  it("falls back to broad browser RootAccepted reconstruction when hinted log lookup fails", async () => {
    const hintedBlock = "0x77" as const;
    const noteRecord = createSandboxSpendMaterialNoteRecord({
      commitment: DEFAULT_AMOUNT_POSEIDON_COMMITMENT,
      noteAmountWei: "10000000000000",
    ...
    setDiagnosticsLocation();
    const destination = "0x1111111111111111111111111111111111111111";
    const withdrawTxHash = `0x${"0b".repeat(32)}`;
    const logFilters: Array<{ fromBlock?: string; toBlock?: string }> = [];
    const request = vi.fn(async ({ method, params }) => {
      if (method === "eth_call") {
        if (params?.[0]?.data === CURRENT_ROOT_CALLDATA) {
      ...
      vi.fn(async (url, init) => {
        ...
        if (key === MEGAETH_TESTNET_RPC_URL) {
          ...
          if (body.method === "eth_getLogs") {
            ...
            if (filter.toBlock === hintedBlock) {
              return Response.json({
                jsonrpc: "2.0",
                id: body.id,
                error: { message: "hinted RootAccepted range unavailable" }
              });
            }
            return Response.json({ jsonrpc: "2.0", id: body.id, result: rootAcceptedLogsFor([DEFAULT_AMOUNT_POSEIDON_COMMITMENT]) });
          }
    ...
    expect(logFilters.some((filter) => filter.toBlock === "latest")).toBe(true);
  });
  ...
  it("selects withdrawal change as the next note after a split public exit", async () => {
    ...
    const firstWithdrawalBlock = "0x88" as const;
    ...
    if (trustedResponse) return trustedResponse;
        ...
        if (key === "http://127.0.0.1:63430/transaction") {
          ...
          return Response.json({
            ...
            receipt: {
              status: "0x1",
              blockNumber: proofRequests.length === 1 ? firstWithdrawalBlock : "0x89",
              from: destination,
              to: SHIELDED_POOL_ADDRESS,
    ...
    const savedChangeRecord = loadSandboxNoteVault(window.localStorage).find(
      (entry) => entry.record.commitment.toLowerCase() === changeCommitment.toLowerCase()
    )?.record;
    expect(savedChangeRecord?.depositBlockNumber).toBe(firstWithdrawalBlock);
  });
  ...
  it("requests an account and typed-data signature before unlocking private balance", async () => {
    ...
    const typedData = JSON.parse(typedDataParams[1]) as {
      ...
      message: { pool: string; warning: string; issuedAt: string };
    };
    ...
    expect(typedData.message.issuedAt).toBe(WALLET_RECOVERY_SCOPE_ISSUED_AT);
    expect(JSON.stringify(typedData.message)).not.toMatch(/appOrigin|sessionNonce|expiresAt/);
