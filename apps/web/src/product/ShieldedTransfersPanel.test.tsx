...
import { ShieldedTransfersPanel } from "./ShieldedTransfersPanel.js";
import {
...
const DEPLOYED_RELAYER_SERVICE_URL = "https://shielded-withdrawal-relayer-testnet.drz-danii.workers.dev/transaction";
...
function createExplicitMainnetProductRuntimeConfig(): ProductRuntimeConfig {
  return {
    ...
    poolDeploymentBlockHex: "0xf87493",
    merkleTreeDepth: 20,
    proverManifestUrl: "/proving/withdraw-artifacts.manifest.json",
    relayerEndpoint: "https://relayer.nullark.com/transaction",
    withdrawVerifierAddress: "0x8112F63A9A9C69BEC73Da0F8618782d11dc8F875",
    withdrawVerifierBytecodeHash: "0xf49079321827e4500f0e051665ce5dae81f58d9fd333c958209680b265bd1cd6",
...
const withdrawProofWorkerRequests: Array<{ id: string; witness: Record<string, string> }> = [];
  ...
  if (key === "/proving/withdraw-artifacts.manifest.json") {
    return Response.json({
      ...
      trustedSetupRecord: {
        ...
        approvedBy: "0x846646aF497d1Df2367F28666257C1a111afF1DA",
...
describe("ShieldedTransfersPanel", () => {
  it("renders only the Nullark deposit and public exit actions by default", () => {
    ...
    expect(screen.getByRole("heading", { name: "Fund private balance" })).toBeInTheDocument();
    expect(screen.getByLabelText("Amount to deposit")).toHaveValue("0.005");
    expect(screen.getByRole("button", { name: "Create private note" })).toBeInTheDocument();
    ...
    expect(screen.queryByLabelText("Submit through local relayer to hide wallet as tx.from")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Send private transfer" })).not.toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Exit to public wallet" })).toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "Send public exit" })).toBeInTheDocument();
    expect(screen.getAllByText("Pool").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Exit to public wallet").length).toBeGreaterThan(0);
  ...
  it("shows withdrawal runtime safety context and wires mobile tabs to panels", () => {
    render(<ShieldedTransfersPanel />);
    ...
    const safetySummary = screen.getByLabelText("Withdrawal runtime safety");
    ...
    expect(safetySummary).toHaveTextContent("MegaETH Testnet");
    expect(safetySummary).toHaveTextContent("0xce4D...C8E1");
    expect(safetySummary).toHaveTextContent("shielded-withdrawal-relayer-testnet.drz-danii.workers.dev");
    expect(safetySummary).not.toHaveTextContent("127.0.0.1:63430");
    expect(safetySummary).not.toHaveTextContent(DEFAULT_BROWSER_WITHDRAW_PROOF);
  });
  ...
  it("fails closed for mainnet value-moving controls until value-moving is approved", () => {
    ...
    expect(screen.getByRole("button", { name: "Create private note" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Send public exit" })).toBeDisabled();
  });
  ...
  it("keeps mainnet value-moving controls enabled only when value-moving is explicitly approved", () => {
    ...
    expect(screen.getByRole("button", { name: "Create private note" })).toBeEnabled();
    expect(screen.getByRole("button", { name: "Send public exit" })).toBeEnabled();
  });
  ...
  it("shows only mainnet public exit choices that leave spendable private change", async () => {
    ...
    const destination = "0x846646aF497d1Df2367F28666257C1a111afF1DA";
    const record = createSandboxSpendMaterialNoteRecord({
      commitment: DEFAULT_AMOUNT_POSEIDON_COMMITMENT,
    ...
    expect(screen.getByRole("radio", { name: /0.01 ETH Exit full note/i })).toBeInTheDocument();
    expect(screen.getByRole("radio", { name: /0.005 ETH Keep 0.005 ETH private/i })).toBeInTheDocument();
  ...
  it("blocks mainnet public exits from already unsupported private change notes", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Send public exit" }));
  ...
  it("lets users connect, change, and disconnect the visible wallet account", async () => {
    const firstAccount = "0x846646aF497d1Df2367F28666257C1a111afF1DA";
    ...
    expect(document.body).toHaveTextContent("0x8466...F1DA");
  ...
  it("shows popular deposit amounts first, exposes the rest through More, and reads wallet balance", async () => {
    const account = "0x846646aF497d1Df2367F28666257C1a111afF1DA";
  ...
  it("allows deposit and relayed withdrawal without trusted prover metadata", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Create private note" })).toBeEnabled();
    expect(screen.queryByRole("button", { name: "Send private transfer" })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Send public exit" })).toBeEnabled();
  ...
  it("always submits guided public exits through the local relayer instead of the user wallet", async () => {
    ...
    if (trustedResponse) return trustedResponse;
        ...
        if (key === "http://127.0.0.1:63430/transaction") {
          return Response.json({
            ...
            relayer: "0x846646aF497d1Df2367F28666257C1a111afF1DA",
            receipt: {
              status: "0x1",
              transactionHash: withdrawTxHash,
              from: "0x846646aF497d1Df2367F28666257C1a111afF1DA",
    ...
    await waitFor(() => expect(screen.getByText("Saved note restored")).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.getAllByText("0x8466...F1DA").length).toBeGreaterThanOrEqual(2));
  ...
  it("selects withdrawal change as the next note after a split public exit", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Send public exit" })).not.toBeDisabled());
    fireEvent.click(screen.getByRole("button", { name: "Send public exit" }));
