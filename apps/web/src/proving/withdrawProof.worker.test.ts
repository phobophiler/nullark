...
describe("withdraw proof worker artifact trust", () => {
  ...
  it("uses the request runtime config in worker context instead of falling through to mainnet defaults", async () => {
    setProductRuntimeConfigForTests(null);
    const wasmHash = await sha256Text("wasm-bytes");
    const zkeyHash = await sha256Text("zkey-bytes");
    mockFetch(localUntrustedManifestRoutes({ wasmHash, zkeyHash }, "/proving/v1-2-testnet/withdraw-artifacts.manifest.json"));

    vi.spyOn(URL, "createObjectURL")
      .mockImplementationOnce(() => "blob:request-runtime-wasm")
      .mockImplementationOnce(() => "blob:request-runtime-zkey");
    vi.spyOn(URL, "revokeObjectURL").mockImplementation(() => undefined);
    ...
    expect(artifacts.wasmUrl).toBe("blob:request-runtime-wasm");
    expect(artifacts.zkeyUrl).toBe("blob:request-runtime-zkey");
  });
  ...
  it("accepts explicit v1.2 unlinkable trusted setup records with exactly 10 public inputs", async () => {
    const bundle = await trustedManifest({ publicInputOrder: V12_SPEND_PUBLIC_INPUT_ORDER });
    mockFetch(bundle.routes);

    vi.spyOn(URL, "createObjectURL")
      .mockImplementationOnce(() => "blob:v12-wasm")
      .mockImplementationOnce(() => "blob:v12-zkey");
    vi.spyOn(URL, "revokeObjectURL").mockImplementation(() => undefined);

    const artifacts = await assertTrustedProverArtifacts(undefined, "v1.2-unlinkable");

    expect(artifacts.wasmUrl).toBe("blob:v12-wasm");
    expect(artifacts.zkeyUrl).toBe("blob:v12-zkey");
  });

  it("rejects old 12-input trusted setup records in explicit v1.2 unlinkable mode", async () => {
    const bundle = await trustedManifest();
    mockFetch(bundle.routes);

    const createObjectURL = vi.spyOn(URL, "createObjectURL").mockImplementation(() => "blob:should-not-be-created");

    await expect(assertTrustedProverArtifacts(undefined, "v1.2-unlinkable")).rejects.toThrow(
      "trusted-setup-public-input-order-mismatch"
    );
    expect(createObjectURL).not.toHaveBeenCalled();
  });
...
function localUntrustedManifestRoutes(
  input: { wasmHash: string; zkeyHash: string },
  manifestUrl = "/proving/withdraw-artifacts.manifest.json"
