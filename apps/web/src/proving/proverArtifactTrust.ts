...
async function verifyTrustedSetupRecord(input: {
  ...
  if (!publicInputOrderMatches(record.publicInputOrder)) {
...
function publicInputOrderMatches(value: unknown): boolean {
  return (
    Array.isArray(value) &&
    value.length === NULLARK_V1_1_PUBLIC_INPUT_ORDER.length &&
    value.every((entry, index) => entry === NULLARK_V1_1_PUBLIC_INPUT_ORDER[index])
  );
}
