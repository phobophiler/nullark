import { BPS_DENOMINATOR } from "@nullark/core";

const WITHDRAW_PUBLIC_INPUTS_LENGTH = 12;
const V12_UNLINKABLE_WITHDRAW_PUBLIC_INPUT_ORDER = [
  "root",
  "nullifier",
  "outputCommitment",
  "destination",
...
export type V12UnlinkableWithdrawProofIntent = {
  ...
  proofContextHash: `0x${string}`;
  encryptedOutputNoteHash: `0x${string}`;
};
...
export async function generateBrowserWithdrawProof(input: BrowserWithdrawProofInput): Promise<BrowserWithdrawProofResult> {
  ...
  const publicInputSchema = resolveWithdrawProofPublicInputSchema(input.publicInputSchema);
  return {
...
function assertWithdrawWitnessFee(
  witness: Record<string, string | string[] | number | bigint>,
  expectedFeeBps?: number | undefined
): void {
...
export function parseSolidityCalldataProof(
  calldata: string,
  options: { publicInputSchema?: WithdrawProofPublicInputSchema | undefined } = {}
  ...
  const publicInputSchema = resolveWithdrawProofPublicInputSchema(options.publicInputSchema);
  ...
  return {
    ...
    publicInputs:
      publicInputSchema === "v1.2-unlinkable"
        ? assertV12UnlinkableWithdrawPublicInputs(publicSignals.map(toBytes32))
        : assertWithdrawPublicInputs(publicSignals.map(toBytes32))
  };
}
...
export function validateV12UnlinkableWithdrawProofIntent(
  ...
  ] = assertV12UnlinkableWithdrawPublicInputs(publicInputs);

  assertBytes32Equal(
    rootSignal,
    intent.root,
    "v1.2 unlinkable withdrawal proof root does not match the intended root."
  );
  assertBytes32Equal(
    nullifierSignal,
    intent.nullifier,
...
type V12UnlinkableWithdrawPublicInputs = [
  `0x${string}`,
  `0x${string}`,
  `0x${string}`,
  `0x${string}`,
...
export function assertV12UnlinkableWithdrawPublicInputs(
  publicInputs: `0x${string}`[]
): V12UnlinkableWithdrawPublicInputs {
  ...
  return publicInputs as V12UnlinkableWithdrawPublicInputs;
}
...
function resolveWithdrawProofPublicInputSchema(
  ...
  const schema = requestedSchema ?? "v1.1";
