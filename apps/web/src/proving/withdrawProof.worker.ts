...
export async function assertTrustedProverArtifacts(manifestUrl = MANIFEST_URL): Promise<TrustedProverArtifacts> {
  const runtimeConfig = getProductRuntimeConfig();
  const artifactPointers = allowsLocalDevUntrustedProver(runtimeConfig)
    ? await loadLocalDevProverArtifactPointers(manifestUrl)
    : await loadTrustedProverArtifactPointers(manifestUrl, runtimeConfig);
