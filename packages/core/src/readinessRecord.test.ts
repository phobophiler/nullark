...
const v12LaneRefs: V12LaneHashRef[] = v12LaneRefTuples.map(([lane, filename], index) => ({
  lane,
  path: `evidence/mainnet-readiness/v1-2/${filename}`,
  ...
  it("rejects stale lane hash bindings", () => {
    expect(
      collectV12LaneHashBindingBlockers(v12LaneRefs, {
        "evidence/mainnet-readiness/v1-2/custody-record.approved.json": "0".repeat(64)
      })
    ).toContain("v1.2 aggregate readiness hash mismatch for custody-record: evidence/mainnet-readiness/v1-2/custody-record.approved.json");
  });

  it("rejects missing current lane hash evidence", () => {
    const currentHashes = v12CurrentLaneHashes();
    delete currentHashes["evidence/mainnet-readiness/v1-2/source-verification-package.approved.json"];

    expect(collectV12LaneHashBindingBlockers(v12LaneRefs, currentHashes)).toContain(
      "v1.2 aggregate readiness missing current lane hash for source-verification-package: evidence/mainnet-readiness/v1-2/source-verification-package.approved.json"
    );
  });
      ...
      collectV12LaneHashBindingBlockers(
        [
          ...v12LaneRefs,
          {
            ...
            path: "evidence/mainnet-readiness/v1-2/aggregate-readiness.approved.json",
  ...
  it("rejects duplicate lane refs and duplicate paths", () => {
    expect(collectV12LaneHashBindingBlockers([...v12LaneRefs, { ...v12LaneRefs[0] }], {})).toEqual(
      expect.arrayContaining([
        ...
        "v1.2 aggregate readiness laneRefs must not contain duplicate paths: evidence/mainnet-readiness/v1-2/public-runtime-current-state.json"
      ])
    );
  });
  ...
  it("rejects lane/path label mismatches", () => {
    const mismatchedRefs = v12LaneRefs.map((ref) =>
      ref.lane === "source-verification-package"
        ? {
            ...ref,
            path: "evidence/mainnet-readiness/v1-2/deployment-package.approved.json"
          }
        : ref
    );

    expect(collectV12LaneHashBindingBlockers(mismatchedRefs, {})).toContain(
      "v1.2 aggregate readiness laneRefs must not contain lane/path mismatches: evidence/mainnet-readiness/v1-2/deployment-package.approved.json"
    );
  });
