...
export function collectV12CustodyLaneBlockers(
  ...
  if (!record.ownerApprovalRef || (!isPrivateOwnerApprovalRef(record.ownerApprovalRef) && !/^docs\/evidence\/owner-approval\/.+/i.test(record.ownerApprovalRef))) {
...
export function assertAdminCustodyReady(record: AdminCustodyRecord, options: AdminCustodyReadyOptions = {}): AdminCustodyRecord {
  ...
  if (!/docs\/evidence\/megaeth-mainnet-deployment-package\.json$/i.test(record.deploymentPackageRef)) {
