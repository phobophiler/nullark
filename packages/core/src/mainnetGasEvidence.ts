...
function assertMainnetGasEvidenceEntry(entry: MainnetGasEvidenceEntry, chainId: number, rpcUrl: string): void {
  ...
  if (STATE_DEPENDENT_OPERATIONS.has(entry.operation)) {
    ...
    if (!/^evidence\/mainnet-readiness\/v1-2\/.+/i.test(entry.stateReadinessRef)) {
...
function assertVolatileMetadataPathReview(entry: MainnetVolatileMetadataReviewEntry): void {
  ...
  for (const pathReview of pathReviews) {
    ...
    if (pathReview.status === "reviewed-accepted") {
      ...
      if (!isMainnetReadinessEvidenceRef(pathReview.computeLimitEvidenceRef)) {
        throw new Error(`mainnet volatile block metadata path ${pathReview.path} requires compute-limit evidence`);
      }
    }
...
function isMainnetReadinessEvidenceRef(value: string): boolean {
  return /^docs\/evidence\/mainnet-readiness\/.+/i.test(value) || /^public-artifacts\/evidence\/mainnet-readiness\/.+/i.test(value);
}
...
function assertExternalInputRefs(values: readonly string[] | undefined, label: string): void {
  ...
  for (const value of values) {
    ...
    if (!/^evidence\/mainnet-readiness\/v1-2\/.+/i.test(value) && !/^evidence\/owner-approval\/.+/i.test(value)) {
      throw new Error(`mainnet gas evidence ${label} external input refs must live under private evidence refs`);
    }
...
function assertDeploymentPackageRef(value: string): void {
  if (/^evidence\/mainnet-readiness\/v1-2\/.+/i.test(value) && /deployment-package/i.test(value)) {
    return;
  }
...
function assertOwnerApprovalRef(value: string | undefined): void {
  ...
  if (!isPrivateOwnerApprovalRef(value) && !/^evidence\/owner-approval\/.+/i.test(value)) {
    throw new Error("mainnet gas evidence owner approval ref must live under evidence/owner-approval");
  }
}
...
function assertMainnetReadinessRef(value: string, label: string, pattern: RegExp): void {
  if (!/^evidence\/mainnet-readiness\/v1-2\/.+/i.test(value)) {
    throw new Error(`mainnet gas evidence ${label} must live under evidence/mainnet-readiness/v1-2`);
  }
...
function isV12EvidencePath(value: string | undefined): value is string {
  return (
    typeof value === "string" &&
    /^evidence\/mainnet-readiness\/v1-2\/.+/i.test(value) &&
