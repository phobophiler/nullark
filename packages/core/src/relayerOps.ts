...
function assertV12OwnerApproval(ownerApprovalRef: string | undefined, ownerApprovalSha256: string | null | undefined): void {
  ...
  if (!isPrivateOwnerApprovalRef(ownerApprovalRef) && !/^docs\/evidence\/owner-approval\/.+v1-?2.+/i.test(ownerApprovalRef)) {
    throw new Error("v1.2 relayer runtime policy owner approval ref must live under docs/evidence/owner-approval and identify v1.2");
  }
...
function assertV12MainnetReadinessPath(value: string | undefined, label: string): void {
  ...
  if (!/^docs\/evidence\/mainnet-readiness\/v1-2\/.+/i.test(value)) {
    throw new Error(`v1.2 relayer runtime policy ${label} must live under docs/evidence/mainnet-readiness/v1-2`);
  }
}
...
function assertOwnerApprovalRef(value: string | undefined): void {
  ...
  if (!isPrivateOwnerApprovalRef(value) && !/^docs\/evidence\/owner-approval\/.+/i.test(value)) {
    throw new Error("relayer ops record owner approval ref must live under docs/evidence/owner-approval");
  }
}
