...
export function buildUnlockPrivateBalanceTypedData(input: WalletUnlockInput): UnlockPrivateBalanceTypedData {
  ...
  const wallet = assertAddress(input.wallet, "Invalid wallet address.");
  const pool = assertAddress(input.pool, "Invalid shielded pool address.");
...
function assertSupportedMegaEthChain(chainId: number): SupportedMegaEthChainId {
...
function assertIssuedAt(value: string): void {
