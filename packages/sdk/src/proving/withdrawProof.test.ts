...
describe("withdraw proof generation", () => {
  ...
  it("rejects witness fee drift before proving", () => {
    expect(() => assertWithdrawWitnessFee({ grossAmount: "10000", fee: "34" }, 33)).toThrow(
      "Withdrawal witness fee must equal floor(grossAmount * 33 / 10000)."
    );
    expect(() => assertWithdrawWitnessFee({ grossAmount: "10000", fee: "33" })).toThrow(
      "Withdrawal witness expected fee bps must be provided by the active runtime."
    );
  });
