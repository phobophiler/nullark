...
const PACKAGE_PINNED_V12_PROMOTION_EVIDENCE: readonly PackagePinnedV12PromotionEvidence[] = [
  ...
  },
  {
    ...
    sha256: "ec12da21c3c0ecb59cfe4b3e1d9557c5d32e0e39fe69b80417fb21db3b99a98d",
...
function hasVerifiedV12PromotionEvidence(runtime: NullarkCurrentRuntime): boolean {
  ...
  return (
    ...
    Array.isArray(readiness.promotionEvidence) &&
    readiness.promotionEvidence.length > 0 &&
    readiness.promotionEvidence.some(isPackagePinnedV12PromotionEvidence)
  );
}
...
function isPackagePinnedV12PromotionEvidence(evidence: unknown): boolean {
  ...
  return PACKAGE_PINNED_V12_PROMOTION_EVIDENCE.some(
    (pinned) => candidate.path === pinned.path && sha256 === pinned.sha256
  );
}
