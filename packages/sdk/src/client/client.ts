...
import {
  ...
  type RecoveryKitRootAcceptedEvidence
...
export class Nullark {
  readonly notes: {
    ...
    importRecoveryKit: (input: {
      serializedKit: string;
      rootAcceptedEvidence?: RecoveryKitRootAcceptedEvidence;
    }) => Promise<PrintableRecoveredNoteSummary>;
  };
  ...
  private async importRecoveryKit(input: {
    serializedKit: string;
    rootAcceptedEvidence?: RecoveryKitRootAcceptedEvidence;
  }): Promise<PrintableRecoveredNoteSummary> {
    const note = await importRecoveryKitV1ToSpendableNote({
      ...
      rootAcceptedEvidence: input.rootAcceptedEvidence
    });
