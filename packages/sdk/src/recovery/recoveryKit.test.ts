...
describe("RECOVERY_KIT_V1 import", () => {
  ...
  it("fails closed for mainnet recovery kits without verified RootAccepted evidence", async () => {
    const mainnetRuntime = getCurrentRuntime();
    const mainnetSpendMaterial = {
      ...spendMaterial,
      chainId: mainnetRuntime.chainId,
      pool: mainnetRuntime.pool
    };
    const kit = createRecoveryKitV1({
      runtime: mainnetRuntime,
      spendMaterial: mainnetSpendMaterial,
      transactionHash: `0x${"66".repeat(32)}`,
      leafIndex: 8,
      blockNumber: "0x12930a1"
    });
    ...
    await expect(
      importRecoveryKitV1ToSpendableNote({
        serializedKit: serializeRecoveryKitV1(kit),
        runtime: mainnetRuntime,
        deriveCommitment: async () => mainnetSpendMaterial.commitment,
        deriveNullifier: async () => nullifier,
        isNullifierSpent: async () => false
      })
    ).rejects.toThrow("RootAccepted verification is required");
  });

  it("rejects mainnet recovery-kit evidence that does not bind the expected leaf", async () => {
    const mainnetRuntime = getCurrentRuntime();
    const mainnetSpendMaterial = {
      ...spendMaterial,
      chainId: mainnetRuntime.chainId,
    ...
    await expect(
      importRecoveryKitV1ToSpendableNote({
        ...
        rootAcceptedEvidence: {
          ...
          chainId: mainnetRuntime.chainId,
          pool: mainnetRuntime.pool,
          runtimeId: mainnetRuntime.productVersion,
          commitment: mainnetSpendMaterial.commitment,
          leafIndex: 9,
          root: `0x${"77".repeat(32)}`,
          latestCheckedBlock: "0x12930a1"
        }
      })
    ).rejects.toThrow("RootAccepted evidence leaf index does not match");
  });
});
