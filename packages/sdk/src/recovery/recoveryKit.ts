...
export type RecoveryKitImportInput = {
  ...
  rootAcceptedEvidence?: RecoveryKitRootAcceptedEvidence | undefined;
};
...
export type RecoveryKitRootAcceptedEvidence = {
  ...
  leafIndex: number;
  root: HexString;
  latestCheckedBlock?: HexString | undefined;
};
...
export async function importRecoveryKitV1ToSpendableNote(input: RecoveryKitImportInput): Promise<RecoveryKitSpendableNote> {
  ...
  assertRecoveryKitRootAcceptedEvidence(kit, input.runtime, input.rootAcceptedEvidence);
...
function assertRecoveryKitRootAcceptedEvidence(
  kit: RecoveryKitV1,
  runtime: NullarkCurrentRuntime,
  evidence: RecoveryKitRootAcceptedEvidence | undefined
): void {
  if (!evidence) {
    if (runtime.environment === "megaeth-mainnet") {
      throw new Error("Recovery kit RootAccepted verification is required before importing a mainnet spendable note.");
    }
    return;
  }
  if (evidence.status !== "verified-root-accepted") {
    throw new Error("Recovery kit RootAccepted evidence must be verified.");
  }
  if (evidence.chainId !== kit.chainId || evidence.chainId !== runtime.chainId) {
    throw new Error("Recovery kit RootAccepted evidence chain ID does not match.");
  }
  if (evidence.pool.toLowerCase() !== kit.poolAddress.toLowerCase() || evidence.pool.toLowerCase() !== runtime.pool.toLowerCase()) {
    throw new Error("Recovery kit RootAccepted evidence pool does not match.");
  }
  if (evidence.runtimeId !== undefined && evidence.runtimeId !== kit.runtimeId) {
    throw new Error("Recovery kit RootAccepted evidence runtime ID does not match.");
  }
  if (evidence.commitment.toLowerCase() !== kit.commitment.toLowerCase()) {
    throw new Error("Recovery kit RootAccepted evidence commitment does not match.");
  }
  if (evidence.leafIndex !== kit.leafIndexHint) {
    throw new Error("Recovery kit RootAccepted evidence leaf index does not match.");
  }
  requireBytes32(evidence.root, "Recovery kit RootAccepted evidence root");
  if (BigInt(evidence.root) === 0n) {
    throw new Error("Recovery kit RootAccepted evidence root must be nonzero.");
  }
  if (evidence.latestCheckedBlock !== undefined) {
    requireBlockQuantity(evidence.latestCheckedBlock);
    if (kit.blockNumberHint !== null && BigInt(evidence.latestCheckedBlock) < BigInt(kit.blockNumberHint)) {
      throw new Error("Recovery kit RootAccepted evidence does not cover the hinted block.");
    }
  }
}
