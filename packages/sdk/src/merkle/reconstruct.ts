...
export function reconstructMerklePathFromRootAcceptedLogs(input: ReconstructMerklePathInput): ReconstructedMerklePath {
  ...
  const leaves = input.logs
    .map((log) => {
      assertFieldBytes32(log.root, "root");
      assertFieldBytes32(log.previousRoot, "previousRoot");
      return assertFieldBytes32(log.insertedCommitment, "insertedCommitment");
    })
