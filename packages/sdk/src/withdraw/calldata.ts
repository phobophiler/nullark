...
export const NULLARK_V1_2_UNLINKABLE_PUBLIC_INPUTS_LENGTH = 10;
...
export type V12UnlinkableWithdrawPublicInputBinding = {
  publicInputs: readonly string[];
  nullifier: string;
  destination: string;
  grossAmountWei: string;
...
export type DecodedV12UnlinkableWithdrawOutputNoteCalldata = Omit<DecodedStageCWithdrawCalldata, "encryptedChangeNote"> & {
  encryptedOutputNote: HexString;
};

export type V12UnlinkableWithdrawOutputNoteCalldataInput = WithdrawBoundedCalldataInput & {
...
export function encodeV12UnlinkableWithdrawBoundedCalldata(input: WithdrawBoundedCalldataInput): HexString {
  normalizeWithdrawInput(input, assertV12UnlinkableWithdrawPublicInputs);
  throw new Error("v1.2 unlinkable withdrawals require encrypted output note calldata.");
}

export function encodeV12UnlinkableWithdrawOutputNoteCalldata(input: V12UnlinkableWithdrawOutputNoteCalldataInput): HexString {
  const normalized = normalizeWithdrawInput(input, assertV12UnlinkableWithdrawPublicInputs);
  const encryptedOutputNote = normalizeHexBytes(
    input.encryptedOutputNote,
    "Expected proof and encrypted output note to be even-length hex bytes."
  );
  if (encryptedOutputNote === "0x") {
...
export function encodeVerifiedV12UnlinkableWithdrawBoundedCalldata(
  input: WithdrawBoundedCalldataInput &
    Omit<V12UnlinkableWithdrawPublicInputBinding, "publicInputs" | "nullifier" | "destination" | "grossAmountWei">
): HexString {
  assertV12UnlinkableWithdrawPublicInputBinding(input);
  return encodeV12UnlinkableWithdrawBoundedCalldata(input);
}

export function encodeVerifiedV12UnlinkableWithdrawOutputNoteCalldata(
  input: V12UnlinkableWithdrawOutputNoteCalldataInput &
    Omit<V12UnlinkableWithdrawPublicInputBinding, "publicInputs" | "nullifier" | "destination" | "grossAmountWei">
): HexString {
  assertV12UnlinkableWithdrawPublicInputBinding(input);
  return encodeV12UnlinkableWithdrawOutputNoteCalldata(input);
}
...
export function decodeStageCWithdrawCalldata(value: string): DecodedStageCWithdrawCalldata {
  ...
  try {
    ...
    const normalizedPublicInputs = assertDecodedStageCWithdrawPublicInputs(publicInputs);
...
export function decodeV12UnlinkableWithdrawOutputNoteCalldata(value: string): DecodedV12UnlinkableWithdrawOutputNoteCalldata {
  const { encryptedChangeNote, ...decoded } = decodeStageCWithdrawCalldata(value);
  const publicInputs = assertV12UnlinkableWithdrawPublicInputs(decoded.publicInputs);
  const encryptedOutputNote = normalizeHexBytes(
    encryptedChangeNote,
    "Expected proof and encrypted output note to be even-length hex bytes."
  );
...
export function assertV12UnlinkableWithdrawPublicInputBinding({
  publicInputs,
  nullifier,
  destination,
  grossAmountWei,
  ...
  expectedPool,
  expectedChainId
}: V12UnlinkableWithdrawPublicInputBinding): void {
  const [
    proofRoot,
    proofNullifier,
  ...
  if (outputCommitment !== undefined && proofOutputCommitment.toLowerCase() !== outputCommitment.trim().toLowerCase()) {
    throw new Error("Withdrawal output commitment does not match public inputs.");
  }
  if (bytes32ToEvmAddress(proofDestination).toLowerCase() !== expectedDestination.toLowerCase()) {
    throw new Error("Withdrawal destination does not match public inputs.");
  }
...
function normalizeWithdrawInputWithPublicInputShape(
  input: WithdrawBoundedCalldataInput,
  assertPublicInputShape: (values: readonly string[]) => HexString[]
): {
  proof: HexString;
  publicInputs: HexString[];
  nullifier: HexString;
  ...
  const publicInputs = assertPublicInputShape(input.publicInputs);
...
function normalizeWithdrawInput(
  input: WithdrawBoundedCalldataInput,
  assertPublicInputShape?: (values: readonly string[]) => HexString[]
): {
  proof: HexString;
  ...
  return normalizeWithdrawInputWithPublicInputShape(input, assertPublicInputShape ?? assertPublicInputs);
}
...
function assertDecodedStageCWithdrawPublicInputs(values: readonly string[]): HexString[] {
  if (
    values.length !== NULLARK_V1_1_PUBLIC_INPUTS_LENGTH &&
    values.length !== NULLARK_V1_2_UNLINKABLE_PUBLIC_INPUTS_LENGTH
  ) {
    throw new Error("Expected exactly 10 or 12 public input bytes32 values.");
  }
  return values.map((value) => assertBytes32(value, "Expected every public input to be bytes32."));
}

function assertV12UnlinkableWithdrawPublicInputs(values: readonly string[]): HexString[] {
  if (values.length !== NULLARK_V1_2_UNLINKABLE_PUBLIC_INPUTS_LENGTH) {
    throw new Error("Expected exactly 10 public input bytes32 values for v1.2 unlinkable withdrawal.");
  }
  return values.map((value) => assertBytes32(value, "Expected every v1.2 unlinkable public input to be bytes32."));
}
