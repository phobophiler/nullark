...
- Mainnet value-moving is enabled for the current runtime, prover artifacts, and relayer path recorded in the public artifacts.
...
Those private operator packets are intentionally kept outside this repository. Public verification checks prove the source package boundary, not final operator approval or private mainnet readiness.
...
The public package intentionally does not expose `mainnet:readiness:*` scripts. Final mainnet readiness validation belongs to the private ops packet and must be run with the private evidence overlay, not from this source-only checkout.
