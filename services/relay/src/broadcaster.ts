...
export const MEGAETH_MAINNET_APPROVED_WITHDRAWAL_RELAYER_POOL = "0x08bA57aA9Bc13Ccaf0dda0Fb7Cd7A2570b0FE4d8" as const;
export const MEGAETH_MAINNET_APPROVED_WITHDRAWAL_RELAYER_SELECTOR = STAGE_C_WITHDRAW_BOUNDED_SELECTOR;
...
function validateMainnetProductionSelectorPolicy(
  ...
  const approvedPool = normalizeHex(MEGAETH_MAINNET_APPROVED_WITHDRAWAL_RELAYER_POOL);
  const configuredContracts = new Set(policy.allowedContracts.map(normalizeHex));
  for (const configuredContract of configuredContracts) {
    if (configuredContract !== approvedPool) {
      errors.push(`mainnet relay policy includes non-production contract ${configuredContract}`);
    }
  }

  if (normalizeHex(request.to) !== approvedPool) {
    errors.push("target contract is not approved for MegaETH mainnet production relayer");
  }
