...
const MAINNET_CONTRACT = MEGAETH_MAINNET_APPROVED_WITHDRAWAL_RELAYER_POOL;
const WRONG_MAINNET_CONTRACT = "0x2222222222222222222222222222222222222222";
...
describe("relay broadcaster policy", () => {
  ...
  it("rejects non-production mainnet contracts even if a production policy accidentally allowlists them", () => {
    const selector = MAINNET_WITHDRAWAL_RELAYER_SELECTORS[0];
    const decision = validateRelayBroadcastRequest(
      {
        ...validRequest,
        chainId: MEGAETH_MAINNET_CHAIN_ID,
        to: WRONG_MAINNET_CONTRACT,
        data: `${selector}00`
      },
      {
        ...policy,
        allowedChainIds: [MEGAETH_MAINNET_CHAIN_ID],
        allowMegaEthMainnet: true,
        allowedContracts: [WRONG_MAINNET_CONTRACT],
        allowedFunctionSelectors: MAINNET_WITHDRAWAL_RELAYER_SELECTORS
      },
      NOW
    );
    ...
    expect(decision.allowed).toBe(false);
    expect(decision.errors).toContain(`mainnet relay policy includes non-production contract ${WRONG_MAINNET_CONTRACT}`);
    expect(decision.errors).toContain("target contract is not approved for MegaETH mainnet production relayer");
  });
