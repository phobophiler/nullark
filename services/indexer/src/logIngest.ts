...
export function ingestPoolLogs(
  ...
  for (const log of input.logs) {
    ...
    } else if (decoded.eventName === "PrivateTransferNoteCreated" || decoded.eventName === "WithdrawalChangeNoteCreated") {
      addEncryptedNoteEvent(state.encryptedNotes, {
        chainId: input.chainId,
        pool: input.pool,
        eventType: decoded.eventName === "PrivateTransferNoteCreated" ? "private-transfer" : "withdraw-change",
        commitment: decoded.args.commitment,
