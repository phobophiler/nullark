...
import {
  ...
  isRelayEndpointPath,
  REQUIRED_MAINNET_RPC_URL,
  REQUIRED_MAINNET_RELAYER_ENDPOINT_PATH,
  REQUIRED_MAINNET_SHIELDED_POOL_ADDRESS,
  REQUIRED_MAINNET_PUBLIC_RUNTIME_SHA256,
  REQUIRED_MAINNET_TRUSTED_SETUP_SHA256,
  REQUIRED_MAINNET_WITHDRAW_SELECTOR,
...
const currentMainnetPool = REQUIRED_MAINNET_SHIELDED_POOL_ADDRESS;
const currentMainnetSelector = REQUIRED_MAINNET_WITHDRAW_SELECTOR;
...
describe("relayer worker policy", () => {
  ...
  it("keeps mainnet policy pinned to the current public artifact pool and withdraw selector only", () => {
    const policy = buildMainnetRelayPolicy(currentMainnetPool);

    expect(policy.allowedChainIds).toEqual([4326]);
    expect(policy.allowMegaEthMainnet).toBe(true);
    expect(policy.allowedContracts).toEqual([currentMainnetPool]);
  ...
  it("keeps v1.2 runtime allowlists separate from historical pre-v1.2 test fixtures", () => {
    const runtime = buildV12RelayRuntimeForTest(currentV12MainnetRuntimeEnv());
    const productionPolicy = buildMainnetRelayPolicy(currentMainnetPool);
    ...
    expect(runtime.policy.allowedContracts).toEqual([currentMainnetPool]);
    expect(runtime.policy.allowedFunctionSelectors).toEqual([currentMainnetSelector]);
    expect(productionPolicy.allowedContracts).toEqual([currentMainnetPool]);
    expect(productionPolicy.allowedFunctionSelectors).toEqual([...MAINNET_WITHDRAWAL_RELAYER_SELECTORS]);
  });
    ...
    for (const deniedPool of deniedPools) {
      expect(() =>
        buildV12RelayRuntimeForTest({
          ...
          RELAYER_V1_2_WITHDRAW_SELECTOR: currentMainnetSelector,
          RELAYER_V1_2_FEE_SOURCE: "on-chain-feeBps"
        })
      ).toThrow("v1.2 mainnet relayer refuses historical/pre-v1.2 mainnet pool address");

      expect(() =>
        buildRelayRuntime(currentMainnetRuntimeEnv({
          SHIELDED_POOL_ADDRESS: deniedPool,
          RELAYER_V1_2_WITHDRAW_SELECTOR: currentMainnetSelector
        }))
      ).toThrow("v1.2 mainnet relayer refuses historical/pre-v1.2 mainnet pool address");
  ...
  it("refuses arbitrary final-looking pools even when MAINNET_RELAYER_APPROVED=true", () => {
    expect(() =>
      buildRelayRuntime(currentMainnetRuntimeEnv({
        SHIELDED_POOL_ADDRESS: configuredPool
      }))
    ).toThrow(`mainnet relayer pool must match approved current public artifact pool ${currentMainnetPool}`);
  });

  it("refuses wrong selector and unapproved RPC before mainnet relay queueing or gas estimation", () => {
    expect(() =>
      buildRelayRuntime(currentMainnetRuntimeEnv({
        RELAYER_V1_2_WITHDRAW_SELECTOR: "0x12345678"
      }))
    ).toThrow(`mainnet relayer selector must match approved current public artifact selector ${currentMainnetSelector}`);

    expect(() =>
      buildRelayRuntime(currentMainnetRuntimeEnv({
        MEGAETH_MAINNET_RPC_URL: "https://rpc.example.invalid"
      }))
    ).toThrow(`mainnet relayer RPC must match approved MegaETH mainnet RPC ${REQUIRED_MAINNET_RPC_URL}`);
  });

  it("requires mainnet relayer approval to be hash-bound to public runtime artifacts", () => {
    const missingPublicRuntimeHash = currentMainnetRuntimeEnv() as Partial<CurrentMainnetRuntimeEnv>;
    delete missingPublicRuntimeHash.MAINNET_RELAYER_PUBLIC_RUNTIME_SHA256;
    expect(() => buildRelayRuntime(missingPublicRuntimeHash)).toThrow(
      "mainnet relayer requires hash-bound MAINNET_RELAYER_PUBLIC_RUNTIME_SHA256 for public-artifacts/current.json"
    );

    expect(() =>
      buildRelayRuntime(currentMainnetRuntimeEnv({
        MAINNET_RELAYER_PUBLIC_RUNTIME_SHA256: "0".repeat(64)
      }))
    ).toThrow("mainnet relayer MAINNET_RELAYER_PUBLIC_RUNTIME_SHA256 must match approved public-artifacts/current.json SHA-256");

    const missingTrustedSetupHash = currentMainnetRuntimeEnv() as Partial<CurrentMainnetRuntimeEnv>;
    delete missingTrustedSetupHash.MAINNET_RELAYER_TRUSTED_SETUP_SHA256;
    expect(() => buildRelayRuntime(missingTrustedSetupHash)).toThrow(
      "mainnet relayer requires hash-bound MAINNET_RELAYER_TRUSTED_SETUP_SHA256 for apps/web/public/proving/trusted-setup-record.json"
    );

    expect(() =>
      buildRelayRuntime(currentMainnetRuntimeEnv({
        MAINNET_RELAYER_TRUSTED_SETUP_SHA256: "f".repeat(64)
      }))
    ).toThrow(
      "mainnet relayer MAINNET_RELAYER_TRUSTED_SETUP_SHA256 must match approved apps/web/public/proving/trusted-setup-record.json SHA-256"
    );
  });
  ...
  it("does not silently fall back to testnet defaults for v1.2 runtime policy", () => {
    ...
    expect(() =>
      buildV12RelayRuntimeForTest({
        RELAYER_ENVIRONMENT: "megaeth-mainnet",
        SHIELDED_POOL_ADDRESS: currentMainnetPool
      })
  ...
  it("resolves v1.2 fee policy from publicClient fee-state reads", async () => {
    ...
    const resolved = await resolveRelayRuntimeFeePolicy(runtime, {
      async readContract(args) {
        ...
        expect(args.address).toBe(currentMainnetPool);
  ...
  it("normalizes v1.2 Stage C calldata against the active on-chain fee policy", () => {
    const runtime = withV12FeePolicy(buildV12RelayRuntimeForTest(currentV12MainnetRuntimeEnv()), { activeFeeBps: 50n });
    const deadline = now + 30;

    const normalized = normalizeRelayRequestForTest(
      {
        chainId: 4326,
        to: currentMainnetPool,
        value: "0",
        data: encodeStageCWithdrawCalldata({
          pool: currentMainnetPool,
          chainId: 4326,
          fee: v12ActiveFee,
  ...
  it("rejects old 12-input private-change calldata through the v1.2 worker normalization path", () => {
    const runtime = withV12FeePolicy(buildV12RelayRuntimeForTest(currentV12MainnetRuntimeEnv()), { activeFeeBps: 33n });
    const deadline = now + 30;
  ...
  it("rejects pending-fee calldata before activation through the v1.2 worker normalization path", () => {
    const runtime = withV12FeePolicy(buildV12RelayRuntimeForTest(currentV12MainnetRuntimeEnv()), {
      activeFeeBps: 50n,
      pendingFeeBps: 75n,
  ...
  it("allows v1.2 unlinkable withdraw signing in mainnet mode only after exact public artifact approval", () => {
    const runtime = withV12FeePolicy(buildRelayRuntime(currentMainnetRuntimeEnv()), { activeFeeBps: 33n });
    const deadline = now + 30;
  ...
  it("rejects historical pre-v1.2 12-input full-exit calldata in mainnet mode", () => {
    const runtime = withV12FeePolicy(buildRelayRuntime(currentMainnetRuntimeEnv()), { activeFeeBps: 33n });
    const deadline = now + 30;
  ...
  it("requires explicit approval and pool configuration before mainnet runtime can be built", () => {
    ...
    const runtime = buildRelayRuntime(currentMainnetRuntimeEnv());

    expect(runtime).toMatchObject({
      environment: "megaeth-mainnet",
      chainId: 4326,
      rpcUrl: REQUIRED_MAINNET_RPC_URL,
      pool: currentMainnetPool
    });
  ...
  it("refuses to build mainnet runtime for the historical ShieldedPoolDepth20 address", () => {
    expect(() =>
      buildRelayRuntime({
        ...currentMainnetRuntimeEnv(),
      ...
      normalizeRelayRequestForTest(
        {
          ...
          to: currentMainnetPool,
          value: "0",
          data: encodeStageBWithdrawCalldata({ pool: currentMainnetPool, chainId: 4326, deadlineOrZero: BigInt(deadline) }),
          deadlineEpochSeconds: deadline
        },
        runtime,
        relayer
  ...
  it("rejects the historical relay route on mainnet while keeping it for non-mainnet mode", () => {
    expect(isRelayEndpointPath(REQUIRED_MAINNET_RELAYER_ENDPOINT_PATH, { environment: "megaeth-mainnet" })).toBe(true);
...
function currentMainnetRuntimeEnv(overrides: Partial<CurrentMainnetRuntimeEnv> = {}): CurrentMainnetRuntimeEnv {
  return {
    RELAYER_ENVIRONMENT: "megaeth-mainnet" as const,
    MAINNET_RELAYER_APPROVED: "true",
    MEGAETH_MAINNET_RPC_URL: REQUIRED_MAINNET_RPC_URL,
    SHIELDED_POOL_ADDRESS: currentMainnetPool,
    MAINNET_RELAYER_PUBLIC_RUNTIME_SHA256: REQUIRED_MAINNET_PUBLIC_RUNTIME_SHA256,
    MAINNET_RELAYER_TRUSTED_SETUP_SHA256: REQUIRED_MAINNET_TRUSTED_SETUP_SHA256,
    RELAYER_V1_2_WITHDRAW_SELECTOR: currentMainnetSelector,
    RELAYER_V1_2_FEE_SOURCE: "on-chain-feeBps" as const,
    ...overrides
  };
}

function currentV12MainnetRuntimeEnv(
  overrides: Partial<Omit<CurrentMainnetRuntimeEnv, "MAINNET_RELAYER_APPROVED">> = {}
): Omit<CurrentMainnetRuntimeEnv, "MAINNET_RELAYER_APPROVED"> {
  return {
    RELAYER_ENVIRONMENT: "megaeth-mainnet" as const,
    MEGAETH_MAINNET_RPC_URL: REQUIRED_MAINNET_RPC_URL,
    SHIELDED_POOL_ADDRESS: currentMainnetPool,
